...
## Failover e URL lock
AUTO detecta somente keys presentes e configuração válida, ordena a cadeia uma vez no início e não reintroduz provider quarantined. Falha antes de resultado válido permite fallback na mesma URL. O primeiro resultado válido fixa PINNED_TO_URL; demais devices usam esse provider. Se o pinned falhar no outro device, não há provider alternativo para aquela URL; a lacuna é DEGRADED/UNKNOWN e o provider é quarantined para próximas URLs.

Provider explícito nunca faz failover cruzado. Após falha qualificadora, o provider explícito fica `QUARANTINED_FOR_AUDIT` e não é consultado novamente no mesmo audit. Nesse modo o status operacional é `DEGRADED` quando a semântica necessária não foi atendida; `CHAIN_EXHAUSTED` é reservado à estratégia AUTO quando todos os providers elegíveis foram quarantined.
...
Além da persistência, logging operacional sanitizado pode registrar por tentativa/sessão provider/model/status/duração/tokens/custo/error_class conforme o `log_level`. O logging não deve incluir API key, Authorization ou corpo integral da requisição. A baseline não exige nem cria `audit.log` persistente; a fonte persistente de telemetria continua sendo `audit.db` + projeção em `report.html`.

## Reporting
report.html distingue habilitado/configurado/tentado/sucesso, estratégia, provider inicial/efetivo, modelo/depth/status/failover e tabela de uso. remediation.html contém somente contexto operacional, nunca finding/recommendation por falha da IA. Em AUTO, chain exhausted adiciona `AI_PROVIDER_CHAIN_EXHAUSTED`, mantém determinísticas e UNKNOWN para dependências semânticas, sem penalidade de score.
